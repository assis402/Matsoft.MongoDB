...
public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : BaseEntity
{
    ...
    public virtual async Task<TEntity> FindByIdAsync(string id, IClientSessionHandle session = null)
    {
        var filter = BaseEntity.FindByIdDefinition<TEntity>(id: id);
        return await FindOneAsync(filterDefinition: filter, session: session); 
    }
    
    public virtual async Task<IEnumerable<TEntity>> FindAsync(FilterDefinition<TEntity> filterDefinition, IClientSessionHandle session = null)
    {
        var result = await _entityCollection.FindAsync(filter: filterDefinition, session: session);
        return await result.ToListAsync();
    }

    public virtual async Task<TEntity> FindOneAsync(FilterDefinition<TEntity> filterDefinition, IClientSessionHandle session = null)
    {
        var result = await _entityCollection.FindAsync(filter: filterDefinition, session: session);
        return await result.FirstOrDefaultAsync();
    }

    public virtual async Task<bool> Exists(FilterDefinition<TEntity> filterDefinition, IClientSessionHandle session = null)
    {
    ...
    public virtual async Task DeleteOneAsync(string id, IClientSessionHandle session = null)
    {
